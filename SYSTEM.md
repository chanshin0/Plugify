...
> 마지막 갱신: 2026-08-19 다중 기기 작업공간 계약 확정. `Plugify`·`second_brain`·`godowon-office`를 임의 절대경로 아래 독립 형제 Git으로 두고, 비형제 배치만 `*_HOME` override를 쓰는 fail-closed migration/resolver를 `docs/WORKSPACE_MIGRATION.md`와 `scripts/workspace-migrate.py`에 기록했다.
