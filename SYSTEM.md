...
> 마지막 갱신: 2026-08-20 다중 기기 SessionStart 최신화 추가. 기기별 최초 bootstrap 뒤 Codex·Claude `startup|resume`에서 잠금→세 형제 Git 검증·fast-forward→최신 Plugify 자산 재생성을 직렬 실행하고, `clear|compact`는 로컬 agent sync만 수행한다. 계정 로그인은 로컬 훅·clone·credential·trust를 배포하지 않는다는 경계를 `docs/WORKSPACE_MIGRATION.md`에 기록했다.
