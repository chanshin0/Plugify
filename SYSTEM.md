...
5. **evals 확충** — ✅ **live-verify case-01 신설·합격(2026-06-25)**(push 미실재→P0 정지; eval 이 SKILL P0 갭 발견·수정). **service-planning 케이스는 아직 0**(다음 후보 — 단 기획 품질 채점이 퍼지해 설계 난이도 높음). live-verify 후속 case-02 후보 = P3 실패→표준 버그 블록 양식. 우선순위는 수정 빈도 순. (perf-review case-01 은 2026-06-25 첫 실행 합격 = 백로그 #1 완료)
