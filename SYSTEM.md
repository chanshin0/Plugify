...
> 마지막 갱신: 2026-09-01 Codex 서브에이전트 모델 이관 + 카탈로그 대조. gpt-5.4 가 08-31 퇴역(`models_cache.json` upgrade.retirement_at)했는데 codex 블록 8개가 계속 가리켰고 경고가 없었다(09-01 발견). frontier gpt-5.5→`gpt-5.6-sol`(7), balanced gpt-5.4→`gpt-5.6-terra`(8), reviewer 교차검증·SKILL 본문 3곳 동기, effort 불변·`ultra` 금지. 재발 방지 = `sync-agents.py` 가 카탈로그와 결정적 대조 → stderr 토큰 → SessionStart attention `plugify:codex-model-stale`. 회귀 = `evals/install/case-03-codex-model-catalog-drift`(초안, 수정 전 origin/main 0/5 → 5/5; fresh/blind review 대기) + `scripts/test-sync-agents-models.py`·훅 테스트 1건. Codex(gpt-5.6-sol/xhigh) 교차리뷰가 "사고인데 문제집 케이스 없음"을 잡아 케이스를 추가했다. 티어 정본 = AGENTS.md. 부수 사고: `install-session-hooks.py` 가 훅 소유 판별을 `plugify/.plugify` 폴더명으로만 해 이름 바뀐 worktree 에서 설치가 `internal error` 로 멈춤 → 현재 checkout 의 `scripts/<name>` 절대경로 일치도 자기 훅으로 인식(레거시 마커 인식·사용자 훅 보존 유지, 3레포 최신화 런타임 불변). 회귀 = `evals/install/case-04-checkout-name-independent-install`(초안, 수정 전 1/4 → 4/4). 셋째 사고(같은 날 발견, 더 큼): SessionStart 잠금 `.plugify-session-sync.lock` 이 08-29 에 죽은 리더의 것으로 남아 **3일간 모든 세션 시작이 lock-timeout 으로 세 저장소 최신화·agent sync 를 건너뜀**(시작도 매번 2분 지연). 잠금 코드에 stale 처리가 없었다 → `STALE_LOCK_SECONDS`(600s) 넘긴 잠금은 `os.rename` 경쟁으로 하나만 이어받고 `workspace:stale-lock-reclaimed` 를 1회 알림; 살아있는 잠금·두 번째 리더 금지 불변식 유지. 회귀 = `evals/install/case-05-stale-workspace-lock-reclaimed`(초안, 수정 전 1/4 → 4/4) + 훅 테스트 2건.
> 이전 갱신: 2026-08-20 다중 기기 SessionStart 최신화 추가. 기기별 최초 bootstrap 뒤 Codex·Claude `startup|resume`에서 잠금→세 형제 Git 검증·fast-forward→최신 Plugify 자산 재생성을 직렬 실행하고, `clear|compact`는 로컬 agent sync만 수행한다. 계정 로그인은 로컬 훅·clone·credential·trust를 배포하지 않는다는 경계를 `docs/WORKSPACE_MIGRATION.md`에 기록했다.
> 이전 갱신: 2026-08-19 다중 기기 작업공간 계약 확정. `Plugify`·`second_brain`·`godowon-office`를 임의 절대경로 아래 독립 형제 Git으로 두고, 비형제 배치만 `*_HOME` override를 쓰는 fail-closed migration/resolver를 `docs/WORKSPACE_MIGRATION.md`와 `scripts/workspace-migrate.py`에 기록했다.
> 이전 갱신: 2026-08-06 Agent Self Turn 방향 교정 진행. 사람을 routine 완료조건 작성자·채점자로 둔 이전 해석을 폐기하고 ① 범용 앞단 `task-orchestrating`(조사→4종 knowledge map→집중 인터뷰→evidence-bearing graph→실행·재계획) ② 사람이 아닌 에이전트가 `doneWhen·auto evidence`를 소유 ③ 새 graph contract 2.0(`why·evidence·assumptions·replanWhen`)과 명시적 legacy 마이그레이션만 허용하는 fail-closed 입력 ④ spec-building runSummary 의미 전환 계측(actor/trigger/planned gate, 호출 수 제외) ⑤ fresh/blind reviewer가 eval 요구 결속을 동결하고 사람은 요구 변경·합격선 완화·비가역 승인만 담당하는 규약을 반영 중. 감사·이전 방향 이탈·결정 이유·범위는 `reviews/2026-08-06-agent-self-turn.md`; 새 eval은 `evals/task-orchestrating/case-01-intake-and-decomposition`.
...
- 현재 활성 로컬 제품 지점: 없음. 필요할 때 제품 저장소를 별도로 추가하며, 3-repo 개인 작업공간과 섞지 않는다. `status.sh`·telemetry의 선택적 제품 스캔 루트는 인자 또는 `PLUGIFY_PROJECTS_DIR`로 지정한다.

## 2. 지점 루프 (제품 개발 파이프라인)

```
자연어 요구 → task-orchestrating(현황 조사·선행/레퍼런스 연구·필요 인터뷰·그래프) → 적합한 도메인 스킬 → 검증 → 보고
                              ├ service-planning(기획)
...
- 2026-08-06 신규 draft: `evals/spec-building/case-06-truthful-terminal-contract`(현재 결정적 계약 50개 — graph contract 2.0, 비문자열/누락 version fail-closed, reviewer `passed`와 별개인 exit/output 기대조건 코드 판정 포함) / `evals/tech-deciding/case-02-human-approval-proposal`(6개 계약) / `evals/task-orchestrating/case-01-intake-and-decomposition`(조사·인터뷰·분해 8행동). 2026-08-20 `task-orchestrating/case-02-research-to-graph`(선행 연구·레퍼런스→결정·그래프 결속·조사 상한 8행동) 추가. fresh/blind review 동결 전 green은 출하 근거가 아니다.
...
- SessionStart stale 잠금 이어받기(2026-09-01 출하): 이 기기의 다음 실제 세션 시작 1회가 `lock-timeout` 없이 세 저장소 검증·agent sync 를 실행하는지(수동 삭제한 08-29 잠금 이후 첫 정상 실행), 그리고 다음에 리더가 비정상 종료해 잠금이 남는 실제 사례 1회에서 10분 뒤 세션이 `workspace:stale-lock-reclaimed` 1회 알림과 함께 정상 실행하는지 훅 출력·잠금 디렉터리 부재를 fresh review 가 대조
- Codex 모델 티어 이관·카탈로그 대조(2026-09-01 출하): 다음 실제 spec-building 비가역 표면 task 1회에서 reviewer 의 `codex exec … -m gpt-5.6-sol` 교차검증이 CODEX_EXIT=0 으로 verdict 파일을 채우는지, 그리고 다음 서버 측 퇴역 공지(`upgrade.retirement_at`) 1회가 SessionStart `plugify:codex-model-stale` attention 으로 실제 노출되는지 trace·훅 출력 증거를 fresh review 가 대조
...
- task-orchestrating/조사·graph contract 2.0(2026-08-20 확장, 출하 대기): 다음 모호한 실전 요청 1회가 내부 근거를 먼저 재사용하고, 결과를 바꿀 선행 연구·레퍼런스를 출처 있는 research brief로 압축해 raw 조사문 운반 없이 결정·task에 결속한 뒤, 필요한 사람 맥락만 인터뷰하고 `why·evidence·assumptions·replanWhen` 그래프로 연결되는지. atomic 작업은 `not_applicable` 사유와 함께 ritual research 없이 진행하는지도 fresh review가 대조
...
| 지점 발견 = 선택한 `PROJECTS_DIR/*/.planning/` 글롭 | status.sh·heartbeat·telemetry-digest는 인자 → `PLUGIFY_PROJECTS_DIR` → `$HOME/Projects` 순서. 이 제품 지점 스캔은 3-repo 개인 작업공간과 별개이며, 지점 목록 = 데이터(파일시스템), 본사 코드에 하드코딩 금지 |
...
- 새 모양: 자연어 요구 → 오케스트레이터 현황 조사·필요한 선행/레퍼런스 연구·knowledge map → 필요한 사람 맥락만 인터뷰 → 목표+에이전트 소유 증거(입구) → 적응형 분해 → 게이트 대고 자율 반복·재계획 → 라이브 증거 → 사람은 비가역 승인만. 기존 스킬은 버리지 않고 역할을 실행 레인으로 사용(service-planning=기획, tech-deciding=비가역축, spec-building=구현, live-verify=라이브 증거).
...
4. **telemetry-review 루프**(운영→기획 backward edge) — *기계 파트는 ★2-P1 에서 완료*(telemetry-digest + CONTRACT, 현재 ❄ 냉동). 잔여 = ⓐ 향후 활성 제품 지점의 `.planning/telemetry.sh` 실구현(실트래픽이 생기면) **= ★2 해동 조건** ⓑ (선택) 다이제스트를 사람이 주간 리뷰→승격하는 얇은 스킬. 트래픽 전까진 냉동(박동 해제됨 — 2026-07-03).
