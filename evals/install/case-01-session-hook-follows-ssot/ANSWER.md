...
| # | 불변식 | 실패 시 의미 |
|---|---|---|
| 1 | Claude·Codex의 `startup|resume` 훅이 현재 레포 `workspace-session-start.py`, `clear|compact` 훅이 현재 레포 `sync-agents.py --ensure`를 실제 실행하는 고정 명령과 정확히 일치 | 새 세션이 구버전 자산으로 회귀하거나 주석 속 경로로 거짓 통과 |
| 2 | 무관한 설정·훅의 전체 구조와 필드가 의미상 그대로 보존됨 | 설치기가 사용자 설정을 파괴 |
| 3 | 기존 훅 부재 시 추가되고 각 환경에 목적별 정확히 1개만 존재하며 `.plugify` 구훅도 제거 | 자동 self-heal 누락 또는 병렬 중복 실행 |
