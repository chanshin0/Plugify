...
현재 대화·지정 파일·기존 폴더에서 먼저 조사하고, 결과를 크게 바꾸는 사람만의 의도·청중·제약이 비어 있을 때만 1~3개 집중 질문을 해. 조사 가능한 경로·슬러그·목록 형식을 사용자에게 묻지 마.
...
1. **대상 폴더** — **기본 경로: `~/Documents/presentation-slides/<slug>/`** (사용자가 다른 위치를 명시하지 않는 한 항상 여기로 저장).
   ...
   - 사용자가 슬러그를 안 주면 주제에서 자동 결정. 충돌하거나 기존 폴더를 덮을 위험이 있을 때만 다른 slug를 선택하거나 승인 경계로 올림
...
- 슬라이드 목록을 에이전트가 도출해 실행 계약에 기록하고 진행 업데이트로 알린다. 청중·발표 목적에 따라 이야기 구조가 크게 갈리는데 근거가 없을 때만 권장안과 대안을 붙여 집중 인터뷰한다.
...
2. **슬라이드 목록 확정** — 번호, slug, 제목, 섹션, 레이아웃 타입을 에이전트가 정하고 표로 기록. load-bearing human-context가 없으면 확인 대기 없이 진행
...
- 첫 캡처 후 에이전트가 이미지를 직접 열어 폰트 누락·잘림·opacity 잔여·그라디언트 렌더를 검사하고, 실패하면 수정·재캡처한다. 미학적 선호가 원 요구에 없고 결과를 크게 바꿀 때만 시안을 보여 집중 인터뷰한다.
