...
- **빈 껍데기 먼저(2026-07-21 — 새 산출물 유형/새 지점 한정)**: 프로젝트가 *처음* 내보내는 형태의 산출물(첫 배포, 첫 제출물, 새 전달 형식)이면 내용 구현 전에 골격 task 를 먼저 태운다 — 수용 규격을 역산한 빈 구조가 통과에 치명적인 조건(로드되나·배포되나·로그 남나)을 내용 없이 실증(hello 수준)하고 사람이 확인한 뒤 내용 task 진입. 이후 산출물은 이미 "되는 구조" 안에 떨어진다. 기존 유형의 반복 작업엔 적용하지 않는다(과공정 금지). 원칙 정본 = 본사 AGENTS.md 설계 원칙.
