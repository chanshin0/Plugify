...
- **커밋·푸시 하지 않는다** — 변경은 작업트리에 남긴다(스테이징·커밋은 상위 워크플로우의 Commit 단계 소관). 코드를 미리 커밋하면 Commit 단계의 STATE 커밋과 **2분할**돼 *한 task = 한 atomic 커밋*이 깨진다(2026-06 첫 실전 관찰 관찰). 예외 = 호출 프롬프트가 *명시적으로* 커밋을 지시할 때만(spec-building 워크플로우는 지시하지 않는다).
