...
- For 07–26, accept only RGB 1920×1080 PNGs produced and fresh-verified by the sibling `godowon-office` locked visual entrypoints. Testimony requires `testimony-thumbnail/v1` and `testimony-backplate/v1`; hymn listening requires `hymn-listening-backplate/v1` plus the existing listening-thumbnail PSD v2.1. Reject arbitrary images, stripped manifests, profile/title mismatch, chapter numbers, and any pixel drift.
...
Both profiles require `caption_delivery: youtube-sidecar-srt/v1` and `subtitle_language: ko`. They never rasterize dialogue or lyrics into video frames. The locked backplate contains only the PSD-derived non-caption design elements for its profile, including the exact series constant `'고도원의 찬송편지'` and the chapter-number-free song title. It is generated before candidate intake and then byte-bound to that candidate.
...
2. Inventory approved inputs and ask only for missing human-owned choices or authority. Do not ask the user to perform routine checks the CLI can perform.
   Before candidate intake, render and `verify-run` the profile's required visual assets with the locked office runtime. Testimony text inputs are a human-supplied context line, song title, and exactly two explicit hook lines; hymn-listening backplate takes the song title, while its separate thumbnail takes the human-supplied one- or two-line hook. Never auto-write, auto-wrap, auto-scale, or infer those strings.
...
The candidate contract now includes one real speech-master-style M4A pre-render observation: it exposed that a fresh file's movie timescale cannot be copied from production 03/05 and must be probe-bound. That is not a completed pair observation. Do not call it production-ready from `validate-candidate` alone: the first real pair must still complete intake preflight, locked visual generation, candidate render, semantic QC, human listening/visual approval, and an independent re-render comparison under fresh review. Missing real text/audio inputs or voice authority means this observation remains open rather than silently assumed.
